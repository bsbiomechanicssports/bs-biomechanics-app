...
async function kullaniciBilgiYukle(kullanici) {
  ...
  if (!snap.exists()) {
    // İlk giriş — kullanıcı oluştur
    const tumSnap = await getDocs(collection(db, 'users'));
    const rol = tumSnap.empty ? 'admin' : 'staff'; // İlk kullanıcı admin
    const yeniKullanici = {
    ...
    mevcutKullaniciBilgi = { ...snap.data(), id: kullanici.uid };
  }
}
...
function uygulamaBaslat() {
  ...
  const initials = ad.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);
  document.getElementById('kullanici-avatar').textContent = initials;
  document.getElementById('profil-avatar-buyuk').textContent = initials;
...
async function kullanicilariYukle() {
  ...
  snap.docs.forEach(d => {
    ...
    el.innerHTML = `
      <div class="kullanici-avatar" style="width:36px;height:36px;font-size:13px;">${initials}</div>
      <div class="kullanici-satir-bilgi">
        <div class="kullanici-satir-ad">${u.name || 'İsimsiz'}</div>
        <div class="kullanici-satir-email">${u.email || ''}</div>
      </div>
      <select class="rol-secici" onchange="rolDegistir('${d.id}', this.value)">
...
window.profilKaydet = async () => {
  ...
  await updateDoc(doc(db, 'users', mevcutKullanici.uid), { department: dep });
...
function davetLinkOlustur() {
  const link = `${window.location.origin}?davet=1`;
...
window.etkinlikKaydet = async () => {
  ...
  try {
    if (id) {
      ...
      await addDoc(collection(db, 'events'), veri);
      ...
      toast('Etkinlik oluşturuldu', 'basari');
    }
    modalKapat();
    bugunEtkinlikleriYukle();
    takvimCiz();
