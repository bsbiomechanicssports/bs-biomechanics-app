...
window.navGit = (hedef) => {
  ...
  navGuncelleUI(hedef);
  // Sohbet panelindeyken mesajlar'a dönünce mesajDinleyiciyi kapat
  if (hedef !== 'sohbet' && mesajDinleyici) {
    mesajDinleyici();
    mesajDinleyici = null;
  }
};
    ...
    if (ayarListe && !document.getElementById('gcal-btn')) {
      ...
      gcalDiv.innerHTML = `
        ...
        <button id="gcal-btn" onclick="gcalBagla()" style="
          ...
          Google Takvime Bagla
        </button>
        <p style="font-size:11px;color:var(--yazi-dim);line-height:1.5">
          Bağlandıktan sonra oluşturduğunuz tüm etkinlikler Google Takviminize otomatik eklenir.
        </p>`;
...
window.etkinlikKaydet = async () => {
  ...
  try {
    if (id) {
      ...
      toast('Etkinlik oluşturuldu' + (gcalToken ? ' (Google Takvime eklendi)' : ''), 'basari');
    }
    modalKapat();
    bugunEtkinlikleriYukle();
    takvimCiz();
...
window.gcalBagla = () => {
  if (!tokenClient) {
    toast('Google servisleri yükleniyor, 2 saniye bekleyin...', '');
    setTimeout(() => {
      if (tokenClient) {
        tokenClient.requestAccessToken({ prompt: '' });
      } else {
        toast('Sayfa yenileyip tekrar deneyin', 'hata');
      }
    }, 2000);
    return;
  }
...
function gcalButonGuncelle(bagli) {
  ...
  if (bagli) {
    ...
    btn.textContent = 'Google Takvime Bagla';
